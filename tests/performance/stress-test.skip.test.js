...
export default {
  ...
  output: {
    ...
    client: ../../src/api'
      ...
      const result = await runCLI(['generate'], { cwd: tempDir, timeout: 30000 });
      ...
      // Should not crash or hang (completed within timeout)
      expect(executionTime).toBeLessThan(30000);

      console.log('✅ Circular reference test completed');
    }, 35000);

    test('should handle broken YAML syntax gracefully', async () => {
      console.log('🔧 Testing broken YAML syntax handling...');
      
      // Copy broken syntax fixture to temp directory
      const contractPath = join(tempDir, 'api-contract.yaml');
      cpSync(join(FIXTURES_DIR, 'broken-syntax.yaml'), contractPath);
...
export default {
  ...
  output: {
    ...
    client: ../../src/api'
      ...
      const startTime = Date.now();
      const result = await runCLI(['generate'], { cwd: tempDir, timeout: 15000 });
      const executionTime = Date.now() - startTime;
      ...
      // Should fail gracefully with syntax error message
      expect(result.success).toBe(false);
      expect(result.stderr.toLowerCase()).toMatch(/(syntax|parse|yaml|invalid)/);
      
      // Should fail quickly (not hang on broken syntax)
      expect(executionTime).toBeLessThan(15000);

      console.log('✅ Broken YAML syntax test completed');
    }, 20000);
  });
...
export default {
  ...
  output: {
    ...
    test('should handle large API mock server startup', async () => {
      console.log('🚀 Testing large API mock server startup...');
      ...
      const testPort = 19800 + Math.floor(Math.random() * 100);

      const startTime = Date.now();
      
      // Start mock server as child process
      const child = spawn('node', [CLI_PATH, 'mock', '--port', testPort.toString()], {
        cwd: tempDir,
        stdio: 'pipe'
      });
      ...
      const serverPromise = new Promise((resolve, reject) => {
        child.stdout.on('data', (data) => {
          ...
          if (output.includes('Mock server running successfully')) {
            serverStarted = true;
            const executionTime = Date.now() - startTime;
            console.log(`⏱️  Server startup time: ${executionTime}ms`);
            child.kill('SIGTERM');
            resolve({ success: true, output: serverOutput, startupTime: executionTime });
          }
        });
      ...
      console.log('📤 Server output:', result.output);
      
      // Verify server started successfully
      expect(result.success).toBe(true);
      expect(result.output).toContain('Mock server running successfully');
      
      // Startup should be reasonably fast (under 30 seconds)
      if (result.startupTime) {
        expect(result.startupTime).toBeLessThan(30000);
      }

      console.log('✅ Large API mock server test completed successfully');
    }, 40000);
  });
