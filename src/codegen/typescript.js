class TypeScriptGenerator {
  generateInterfaces(schemas, config = {}) {
    const interfaces = [];
  ...
  wrapInFileTemplate(content, _type, _config) {
    const timestamp = new Date().toISOString();
    const header = `// Generated by SpecJet CLI on ${timestamp}
...
${methods.join('\n\n')}
  ...
  private async handleErrorResponse(response: Response): Promise<never> {
    ...
    return this.wrapInFileTemplate(importSection + clientCode, 'client', config);
  }
