class DocumentationGenerator {
  static generateUsageExamples(contractInfo, config) {
    ...
    return `# ${contractInfo.title} - Generated API Client

${contractInfo.description || 'Generated TypeScript API client and types from OpenAPI specification.'}
...
import { 
  ...
  CreateUserRequest, 
  UpdateUserRequest 
} from '${typesPath}/api.js';

// Import API client
import { ApiClient } from '${clientPath}/client.js';
\`\`\`
...
- \`${typesPath}/api.ts\` - TypeScript interfaces for all API data structures
- \`${clientPath}/client.ts\` - Fully typed API client with authentication support
...
${interfaces}`;
  ...
  static getMethodName(endpoint) {
    return endpoint.operationId || `${endpoint.method.toLowerCase()}${endpoint.path.replace(/\//g, '').replace(/{[^}]+}/g, 'ById')}`;
  }

  static getMethodParameters(endpoint) {
    const params = [];
  ...
  static getReturnTypeName(endpoint, schemas) {
    ...
    // Try to find a matching schema name
